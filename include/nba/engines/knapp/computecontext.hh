...
public:
    ...
    int enqueue_memread_op(uint32_t task_id,
                           const host_mem_t host_buf, const dev_mem_t dev_buf,
                           size_t offset, size_t size);
    int enqueue_kernel_launch(//uint32_t task_id,
                              dev_kernel_t kernel, struct resource_param *res);
...
private:
    ...
    RMALocalMemoryPool *_local_mempool_inout[NBA_MAX_IO_BASES];
    RMALocalMemoryPool *_local_mempool_out[NBA_MAX_IO_BASES];
    RMAPeerMemoryPool *_peer_mempool_in[NBA_MAX_IO_BASES];
    RMAPeerMemoryPool *_peer_mempool_inout[NBA_MAX_IO_BASES];
