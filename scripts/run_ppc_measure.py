...
def draw_plot(records, confname, pktsize, base_path='~/Dropbox/temp/plots/nba/'):
    ...
    ax2 = ax.twinx()
    ax2.set_ylabel('CPU Cycles', fontweight='bold')
    #ax2.set_ylim(0, 1)
    ...
    for node_id in range(2):
        x_ind = tuple(1 - r.cpu_ratio for r in records[node_id][:rec_count])
        ...
        ax2.plot(x_ind, ppc_cpu, ls='-', color=line_colors[node_id])
        ax2.plot(x_ind, ppc_gpu, ls='--', color=line_colors[node_id])
        ax2.plot(x_ind, ppc_est, ls=':', color=line_colors[node_id])
